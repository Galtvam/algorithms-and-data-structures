...
class Lista:
	...
	def tirar(self, indice=None):
		...
		if indice is None:
			...
			if indice > 0 or indice < 0:
				...
				if temp.prox is not self.__ultimo:
	...
	def remover(self, contend):
		...
		temp = self.__primeiro
		try:
			while temp.prox.item is not contend:
				temp = temp.prox
		except:
			raise ValueError    
		aux_retorno = temp.prox
		temp.prox = aux_retorno.prox
		aux_retorno.prox = None
		if aux_retorno is self.__ultimo:
			self.__ultimo = temp
		self.__len -= 1
		return aux_retorno.item
	...
	def procurar(self, contend):
		...
		temp = self.__primeiro.prox
		try:
			while temp.item is not contend:
				temp = temp.prox
				indice += 1
			return indice
		except:
